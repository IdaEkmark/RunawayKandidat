...
for a_D in a_D_for:
    ...
    D0max    = 0.1                     # Upper limit of D0 swep
    D0num    = 101                     # Number of D0-steps
    I_re0min = 1e5                     # Lower limit of I_re0 sweep
    I_re0max = 8e5                     # Upper limit of I_re0 sweep
    I_re0num = 7*100+1                 # Number of I_re0-steps
...
for a_D, D0, I_re0, do in zip(legend_a_D, D0_bra_list, I_re0_bra_list, do_bra_list):  # For loop to plot best I_re for every a_D with describing legend
    legend.append('$a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))  # Legend describing a_D, D0 and I_re0
...
legend=['$I_{p,deVries}$']  # Total legend with I_p,deVries as firt element
plt.plot(t, I_p)            # Plots I_p,deVries as first graph
for a_D, D0, I_re0, do in zip(legend_a_D, D0_bra_list, I_re0_bra_list, do_bra_list):   # For loop to plot best I_re an corresponding I_p for every a_D with describing legend
    legend.append('$I_p$: $a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))  # Legend describing a_D, D0 and I_re0
    do.eqsys.I_p.plot()  # Plots DREAM's I_p

    legend.append('$I_{re}$: $a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))  # Legend describing a_D, D0 and I_re0
    n_re_loop = do.eqsys.n_re[:]                          # n_re for the best I_re0 and D0 value
    I_re_loop = n_re_loop * e * c * A_c                   # I_re for the best I_re0 and D0 value
    tid       = np.linspace(0, tMax, num=len(I_re_loop))  # Equivalent time steps for plot
    plt.plot(tid, I_re_loop)                              # Plot I_re
...
legend = ['$I_{p,deVries}$']  # Total legend with I_p,deVries as firt element
plt.plot(t, I_p)              # Plots I_p,deVries as first graph
for a_D, D0, I_re0, do in zip(legend_a_D, D0_bra_list, I_re0_bra_list, do_bra_list):  # For loop to plot I_p corresponding to best I_re for every a_D with describing legend
    legend.append('$a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))  # Legend describing a_D, D0 and I_re0
    do.eqsys.I_p.plot()  # Plots DREAM's I_p
...
# Figure 4
legend = []  # Total legend
for a_D, D0, I_re0, do in zip(legend_a_D, D0_bra_list, I_re0_bra_list, do_bra_list):  # For loop to plot kappa corresponding to best I_re for every a_D with describing legend
    legend.append('$a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))  # Legend describing a_D, D0 and I_re0
...
# Figure 5
legend = []  # Total legend
for a_D, D0, I_re0, do in zip(a_D_for, D0_bra_list, I_re0_bra_list, do_bra_list):  # For loop to plot GammaAva corresponding to best I_re for every a_D with describing legend
    legend.append('$a_D=$' + str(a_D) + ', $D_0=$' + str("%.3f" % D0) + ', \n$I_{re0}=$' + str(I_re0))   # Legend describing a_D, D0 and I_re0
    GammaAva = do.other.fluid.GammaAva[:]               # Avalanche growth rate GammaAva
    tid      = np.linspace(0, tMax, num=len(GammaAva))  # Equivalent time steps for plot
    plt.plot(tid, GammaAva)  # Plots GammaAva
