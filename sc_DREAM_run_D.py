...
def run_DREAM(E_initial,V_loop,T_e,E1,Z_1,n_1,E2,Z_2,n_2,n_re0,B,a,r_0,A0,D_mat,r,Nr,t,tMax,Nt):
    #### DREAM settings ####
    ...
    # Runaway loss
    t_D = np.linspace(0, tMax, Nt+1)
    r_D = np.linspace(0, a, 2)
    D = D_mat
    ds.eqsys.n_re.transport.prescribeDiffusion(drr=D, t=t_D, r=r_D)

    t_A = np.linspace(tMax / 2, tMax / 2, 1)
    r_A = np.linspace(0, a, Nr + 1)
    A  = A0 * np.ones((1, Nr+1))
    ds.eqsys.n_re.transport.prescribeAdvection(ar=A, t=t_A, r=r_A)
